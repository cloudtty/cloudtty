...
func (c *CloudShellReconciler) CreateCloudShellJob(ctx context.Context, cloudshell *cloudshellv1alpha1.CloudShell) error {
	...
	if len(cloudshell.Spec.ConfigmapName) == 0 {
		...
		if err := ctrlutil.SetControllerReference(cloudshell, kcm, c.Scheme); err != nil {
			klog.Errorf("Failed to set owner reference for configmap", "cloudshell", klog.KObj(cloudshell), "err", err)
			return err
		}
	...
	if template, err := util.LoadYamlTemplate("/etc/cloudtty/job-temp.yaml"); err == nil {
		klog.V(2).Infof("Failed to load job template from /etc/cloudtty", "err", err)
		jobTmpl = template
	}
	...
	if err != nil {
		klog.Errorf("Failed to decode job manifest", "err", err)
		return err
	}
	job := obj.(*batchv1.Job)
	...
	if err := ctrlutil.SetControllerReference(cloudshell, job, c.Scheme); err != nil {
		klog.Errorf("Failed to set owner reference for job", "cloudshell", klog.KObj(cloudshell), "err", err)
		return err
	}
...
func (c *CloudShellReconciler) CreateRouteRule(ctx context.Context, cloudshell *cloudshellv1alpha1.CloudShell) error {
	...
	case "", cloudshellv1alpha1.ExposureServiceNodePort:
		...
		if err != nil {
			klog.V(2).Infof("Unable to get master node IP addr", "cloudshell", klog.KObj(cloudshell), "err", err)
		}
	...
	case cloudshellv1alpha1.ExposureIngress:
		...
		if err := c.CreateIngressForCloudshell(ctx, service, cloudshell); err != nil {
			klog.Errorf("Failed create ingress for cloudshell", "cloudshell", klog.KObj(cloudshell), "err", err)
			return err
		}
	...
	case cloudshellv1alpha1.ExposureVirtualService:
		...
		if err := c.CreateVitualServiceForCloudshell(ctx, service, cloudshell); err != nil {
			klog.Errorf("Failed create virtualservice for cloudshell", "cloudshell", klog.KObj(cloudshell), "err", err)
			return err
		}
		accessURL = SetRouteRulePath(cloudshell)
	}
...
func (c *CloudShellReconciler) GetJobForCloudshell(ctx context.Context, namespace string, cloudshell *cloudshellv1alpha1.CloudShell) (*batchv1.Job, error) {
	...
	if err := c.List(ctx, &childJobs, client.InNamespace(namespace), client.MatchingLabels{"ownership": cloudshell.Name}); err != nil {
		return nil, err
	}
	if len(childJobs.Items) > 1 {
		klog.Errorf("Found multiple cloudshell jobs", "cloudshell", klog.KObj(cloudshell))
		return nil, errors.New("Found multiple cloudshell jobs")
	}
	if len(childJobs.Items) == 0 {
		klog.V(4).InfoS("Failed to find cloudshell job", "cloudshell", klog.KObj(cloudshell))
		return nil, nil
	}
...
func (c *CloudShellReconciler) GetMasterNodeIP(ctx context.Context) (string, error) {
	...
	if err := c.List(ctx, &nodelist, client.MatchingLabels{"node-role.kubernetes.io/master": ""}); err != nil || len(nodelist.Items) == 0 {
		return "", err
	}
...
func (c *CloudShellReconciler) GetServiceForCloudshell(ctx context.Context, namespace string, cloudshell *cloudshellv1alpha1.CloudShell) (*corev1.Service, error) {
	...
	if err := c.List(ctx, &childSvcs, client.InNamespace(namespace), client.MatchingLabels{"ownership": cloudshell.Name}); err != nil {
		return nil, err
	}
	if len(childSvcs.Items) > 1 {
		klog.Errorf("Found multiple cloudshell service", "cloudshell", klog.KObj(cloudshell))
		return nil, errors.New("Found multiple cloudshell services")
	}
	if len(childSvcs.Items) == 0 {
		klog.V(4).InfoS("Failed to find cloudshell service", "cloudshell", klog.KObj(cloudshell))
		return nil, nil
	}
	return &childSvcs.Items[0], nil
}
...
func (c *CloudShellReconciler) CreateCloudShellService(ctx context.Context, cloudshell *cloudshellv1alpha1.CloudShell) (*corev1.Service, error) {
	...
	if err != nil {
		klog.Errorf("Failed to decode service manifest", "err", err)
		return nil, err
	}
...
func (c *CloudShellReconciler) CreateIngressForCloudshell(ctx context.Context, service *corev1.Service, cloudshell *cloudshellv1alpha1.CloudShell) error {
	...
	if ingress != nil && apierrors.IsNotFound(err) {
		...
		if err != nil {
			klog.Errorf("Failed to decode ingress manifest", "err", err)
			return err
		}
...
func (c *CloudShellReconciler) CreateVitualServiceForCloudshell(ctx context.Context, service *corev1.Service, cloudshell *cloudshellv1alpha1.CloudShell) error {
	...
	if virtualService != nil && apierrors.IsNotFound(err) {
		...
		if err != nil {
			klog.Errorf("Failed to decode virtualservice manifest", "err", err)
			return err
		}
...
func (c *CloudShellReconciler) UpdateCloudshellStatus(ctx context.Context, cloudshell *cloudshellv1alpha1.CloudShell, phase string) error {
	...
	if err := c.Status().Update(ctx, cloudshell); err != nil {
		klog.Errorf("Failed to update cloudshell status", "cloudshell", klog.KObj(cloudshell), "err", err)
		return err
	}
	return nil
}
