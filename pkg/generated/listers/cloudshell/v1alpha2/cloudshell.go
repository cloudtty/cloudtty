...
type CloudShellLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha2.CloudShell, err error)
...
// List lists all CloudShells in the indexer.
func (s *cloudShellLister) List(selector labels.Selector) (ret []*v1alpha2.CloudShell, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha2.CloudShell))
	})
	return ret, err
}
...
type CloudShellNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1alpha2.CloudShell, error)
...
// List lists all CloudShells in the indexer for a given namespace.
func (s cloudShellNamespaceLister) List(selector labels.Selector) (ret []*v1alpha2.CloudShell, err error) {
...
// Get retrieves the CloudShell from the indexer for a given namespace and name.
func (s cloudShellNamespaceLister) Get(name string) (*v1alpha2.CloudShell, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha2.Resource("cloudshell"), name)
	}
	return obj.(*v1alpha2.CloudShell), nil
}
